...
function ChatConversation({
  ...
  const [showRecallMenu, setShowRecallMenu] = useState<string | null>(null); // 显示撤回菜单的消息ID
  const [viewingRecalledMsg, setViewingRecalledMsg] = useState<ChatMessage | null>(null); // 查看被撤回的消息
  const [oocMode, setOocMode] = useState(false); // OOC强制指令模式
  const endRef = useRef<HTMLDivElement>(null);
  const longPressTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const inputLongPressTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const titleLongPressTimer = useRef<ReturnType<typeof setTimeout> | null>(null); // 标题长按计时器
  ...
  const lastInputTimeRef = useRef<number>(0); // 最后一次输入时间
  const isTypingCheckInterval = useRef<ReturnType<typeof setInterval> | null>(null); // 检查是否正在输入的定时器
  const currentInputRef = useRef<string>(''); // 当前输入框内容的ref，用于定时器中访问
  ...
  const buildSystem = (): string => {
    // OOC模式：直接与模型对话，不使用角色人设
    if (oocMode) {
      return `你现在处于OOC（Out of Character）模式。用户想要直接与你（AI模型）沟通，而不是与角色对话。
...
请用你作为AI助手的身份，理解并执行用户的指令。简洁明确地回复，帮助用户调整对话。
当用户满意后，会退出OOC模式继续角色扮演。`;
    }
...
【强制格式规则】必须严格遵守：
1. 动作必须用 (动作) 格式，禁止使用 *动作* 或其他格式
2. 动作和语言之间必须空一行，这是强制要求
3. 格式示例：
   正确✓：
   你好呀！
...
请确保每条消息内容充实，符合字数要求，不要过于简短或冗长。`;
  ...
  const send = async () => {
    ...
    setInput('');
    currentInputRef.current = ''; // 同步清空ref
    ...
    const triggerAIReply = async () => {
      ...
      try {
        ...
        (async () => {
          ...
          if (media.length) {
            onSend([...messagesWithAssistant, { id: uid(), role: 'assistant', content: '', ts: Date.now(), media }]);
          }
        })();

        // Background NPC & Plot Event detection（异步处理）
  ...
  const startLongPress = (m: ChatMessage) => {
    // 用户消息：显示撤回菜单
    if (m.role === 'user') {
      longPressTimer.current = setTimeout(() => setShowRecallMenu(m.id), 600);
      return;
    }
    // 角色消息：显示内心想法
    if (m.role === 'assistant' && m.innerThought) {
  ...
  // 撤回消息
  const recallMessage = (msgId: string) => {
    const updated = thread.messages.map((m) => {
      if (m.id === msgId) {
        return {
          ...m,
          recalled: true,
          recalledContent: m.content,
          content: '你撤回了一条消息',
        };
      }
      return m;
...
每条消息的字数应该在 ${minWords} 到 ${maxWords} 字之间。`;
  ...
  // 角色状态显示
  const characterStatus = loading ? '正在输入...' : '在线';
  ...
  return (
        ...
        >
          <div className="flex flex-col items-center">
            <div className="flex items-center gap-2">
              {displayTitle}
              {oocMode && <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-500/20 text-red-400">OOC指令</span>}
            </div>
            <div className="text-[11px] txt-faint mt-0.5">{characterStatus}</div>
          </div>
        </div>
      }
      onBack={onBack}
    ...
    >
          ...
          {thread.messages.map((m, index) => {
            ...
            return (
              <div key={m.id}>
                ...
                {m.recalled ? (
                  <div
                    className="text-center text-[12px] txt-faint my-1 cursor-pointer hover:txt-dim transition-colors"
                    onClick={() => setViewingRecalledMsg(m)}
                  >
                    {m.role === 'user' ? '你撤回了一条消息' : `${thread.charAltName || char.name} 撤回了一条消息`}
                  </div>
                ) : /* 拍一拍消息 - 居中小字样式 */
                        ...
                        <>
                          ...
                          {m.content && !m.moneyTransfer && (
                            <div className="flex flex-col gap-1 items-start w-full">
                              <div
                                className={`px-3.5 py-2.5 rounded-2xl text-[14px] leading-relaxed whitespace-pre-wrap ${m.role === 'user' ? 'rounded-br-md text-white font-medium' : 'glass rounded-bl-md'}`}
              ...
              onClick={async () => {
                if (selectedMessages.size > 0) {
                  ...
                  try {
                    const history: ChatMsg[] = [
                      { role: 'system', content: buildSystem() },
                      ...nextMsgs.map((m) => ({ role: m.role, content: m.content } as ChatMsg)),
                    ];
                    const rich = await callChatRich(api.chat, history, { temperature: 0.85, maxTokens: 800 });
                    const assistantMsg: ChatMessage = {
                      id: uid(),
                    ...
                    onSend([...nextMsgs, assistantMsg]);
                  } catch (e) {
                    onSend([...nextMsgs, { id: uid(), role: 'assistant', content: `（出错了：${(e as Error).message}）`, ts: Date.now() }]);
                  } finally {
                    setLoading(false);
                    setThinkingMsg(null);
                  }
        ...
        <div className="space-y-2">
          ...
          <button
            onClick={() => {
              ...
              setInCall(true);
              setCallStartTime(Date.now());
              sendActive('你刚刚主动打视频电话给用户。请自然地开场打招呼，说说为什么突然想视频聊天。10-50字。');
            }}
            className="w-full py-3 rounded-xl glass hover:bg-[var(--accent)] hover:text-white transition-all text-[14px] font-medium flex items-center justify-center gap-2"
          >
            📹 主动打视频
          ...
          <button
            onClick={() => {
              ...
              setInCall(true);
              setCallStartTime(Date.now());
              sendActive('你刚刚主动打语音电话给用户。请自然地开场打招呼，说说为什么突然想语音聊天。10-50字。');
            }}
            className="w-full py-3 rounded-xl glass hover:bg-[var(--accent)] hover:text-white transition-all text-[14px] font-medium flex items-center justify-center gap-2"
          >
            📞 主动打语音
      ...
      {/* 撤回菜单模态框 */}
      <Modal open={!!showRecallMenu} onClose={() => setShowRecallMenu(null)} title="消息操作">
        <div className="space-y-2">
          <button
            onClick={() => showRecallMenu && recallMessage(showRecallMenu)}
            className="w-full py-3 rounded-xl glass hover:bg-red-500/20 hover:text-red-400 transition-all text-[14px] font-medium flex items-center justify-center gap-2"
          >
            撤回消息
          </button>
        </div>
      ...
      {/* 查看被撤回的消息模态框 */}
      <Modal open={!!viewingRecalledMsg} onClose={() => setViewingRecalledMsg(null)} title="已撤回的消息">
        {viewingRecalledMsg && (
          <div className="space-y-3">
            <div className="glass rounded-xl p-4">
              <div className="text-[11px] txt-faint mb-2">
                {viewingRecalledMsg.role === 'user' ? '你' : (thread.charAltName || char.name)} · {formatTimestamp(viewingRecalledMsg.ts)}
              </div>
              <div className="text-[14px] leading-relaxed">
                {viewingRecalledMsg.recalledContent || viewingRecalledMsg.content}
              </div>
            </div>
          </div>
        )}
