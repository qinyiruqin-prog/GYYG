...
export function ClosetScreen({
  ...
  const aiGenerate = async () => {
    setGenerating(true);
    try {
      const sys = '你在生成服装搭配。返回JSON数组，每个元素：{"name":"搭配名","style":"风格","occasion":"场合","pieces":[{"type":"top/bottom/dress/shoes/accessory"}]}。生成3-5套。';
      const data = await askAIJson<Array<{ name: string; style: string; occasion: string; pieces: { type: Outfit['pieces'][0]['type'] }[] }>>(api, sys, '请生成一批日常服装搭配：', { temperature: 0.8, maxTokens: 500 });

      const newOutfits: Outfit[] = (Array.isArray(data) ? data : [data]).map((o) => ({
        id: uid(),
        name: o.name,
        pieces: o.pieces.map((p) => ({ type: p.type, image: SAMPLE_IMAGES[Math.floor(Math.random() * SAMPLE_IMAGES.length)] })),
        style: o.style,
        occasion: o.occasion,
        favorite: false,
        ts: Date.now(),
    ...
    >
      ...
      <Modal open={composing} onClose={() => { setComposing(false); resetForm(); }} title="创建搭配">
        {generatedOutfits.length > 0 ? (
          <div className="mb-4">
            <div className="text-[13px] txt-dim mb-2">💡 AI推荐搭配 - 选择后确认添加</div>
            <div className="space-y-2 max-h-60 overflow-y-auto mb-3">
              {generatedOutfits.map((o, i) => (
                <div key={i} className="tap glass rounded-lg p-2.5 cursor-pointer hover:txt-accent" onClick={() => {
                  setName(o.name);
                  setStyle(o.style);
                  ...
                  <div className="text-[11px] txt-dim">{o.style} · {o.occasion} · {o.pieces.length}件</div>
                </div>
              ))}
            </div>
            <button onClick={() => setGeneratedOutfits([])} className="w-full h-9 rounded-full glass text-[13px] mb-3">返回编辑</button>
          </div>
        ) : (
          <>
            ...
            <div className="flex gap-3">
              <button onClick={aiGenerate} disabled={aiGenerating} className="tap flex-1 h-11 rounded-full glass font-medium disabled:opacity-50 flex items-center justify-center gap-1.5">
                <Sparkles size={16} className="txt-accent" /> {aiGenerating ? '生成中…' : 'AI 推荐'}
              </button>
